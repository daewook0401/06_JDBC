...
public class JDBCExample2 {
	public static void main(String[] args) {
		 ...
		 * 	 ? 자리에 java 값을 대딥할 준비가 되어있는 Statement
		 * 
		 * 장점 1 : SQL에 값 추가가 간단해짐
		 * 
		 * 장점 2 : ? 값 대입 시 자료형에 맞는 리터럴 표기법으로 자동 변환됨
		 * 
		 * ex) ?에 int 123 대입 -> NUMBER 타입 리터럴 (123)
		 * ex) ?에 String 123 대입 -> CHAR 타입 리터럴 ('123')
		 * 														(양쪽에 '' 자동 추가)
		 * 
		 * 장점 3 :
		 ...
		 * 
		 * PreparedStatement는 ?(placeholder)를 이용해서 파라미터(값)만 연결하기 때문에
		...
		try {
			 ...
			 * -> Connection을 생성하면 기본적으로 true 상태
			 * -> 매번 수동으로 끔
			 * 	왜? 개발자가 트랜젝션을 마음대로 제어하기 위해서
			 * */
			conn.setAutoCommit(false);
			/* PreparedStatement 객체 생성 */
			// -> 객체 생성과 동시 SQL을 매개변수로 전달해서 담아둠
			// -> SQL 파악하기 위해서
